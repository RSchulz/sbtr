/*
	SBTR.scala was derived from src/kiama/rewriting/Rewriter.scala from the Kiama distribution.
	@see http://code.google.com/p/kiama
...
/**
	Constructable: Any entity that has substructural elements (children)
					that can be extracted for traversal and which can be
					reconstructed in variant / transformed form.
*/

trait			Constructable
{
	/** Extract the substructural elements of this <b>Constructable</b> */
	def
	deconstruct: IndexedSeq[SBTR#ATerm]

	/** Build a variant form from an altered set of substructural elements */
	def
	reconstruct(original: SBTR#ATerm, children: IndexedSeq[SBTR#ATerm]): SBTR#ATerm

	/** Ascertain how many substructural elements this <b>Constructable</b> has */
	def
	nSE: Int
}
...
trait			SBTR
{
	...
	*/
	def
	strategyf(f: ATerm => Option[ATerm]): Strategy =
		new Strategy {
			def
			apply(t: ATerm) = f(t)
		}
	...
	*/
	def
	strategy(f: PartialFunction[ATerm,Option[ATerm]]): Strategy =
		new Strategy {
			def
			apply(t: ATerm) = {
				if (f isDefinedAt t) {
					val f_t = f(t)
					if (debug) println("strategy success: " + t + " => " + f_t)
					f_t
				}
				else {
	...
	*/
	def
	rule(f: PartialFunction[ATerm,ATerm]): Strategy =
		new Strategy {
			def
			apply(t: ATerm) = {
				if (f isDefinedAt t) {
					val f_t = f(t)
					if (debug) println("rule success: " + t + " => " + f_t)
					Some(f_t)
				}
				else {
	...
	*/
	implicit
	def
	termToStrategy(t: ATerm): Strategy =
		 strategyf(_ => Some(t))
	...
	*/
	def
	query[T](f: PartialFunction[ATerm,T]): Strategy =
		new Strategy {
			def
			apply(t: ATerm) = {
				if (f isDefinedAt t) {
	...
	*/
	val
	failure: Strategy = strategyf(_ => None)
	...
	*/
	val
	id: Strategy = strategyf(t => Some(t))
	...
	*/
	...
	/**
		Constructable term deconstruction.
	*/
	object DTerm
	{
		/**
		...
		*/
		def
		unapply(t: Constructable): Option[(Any,Seq[Any])] =
			Some(t, t.deconstruct)
	}


	/**
		Generic term deconstruction based on Product (hence covering case classes).
	*/
	object PTerm
	{
		/**
		...
		*/
		def
		unapply(t: Any): Option[(Any,Seq[Any])] = {
			t match {
				case p: Product => {
	...
	/**
		...
		sequence of the values that f has returned for the children. This will
		work on any value, but will only decompose Products and Constructables.
		This operation is similar to that used in the Uniplate library.
	*/
	def
	para[T](f: (Any, Seq[T]) => T): Any => T = {
		case PTerm(t, ts) => f(t, ts.map(para(f)))
		case DTerm(t, ts) => f(t, ts.map(para(f)))
	}
	...
	*/
	private
	def
	dup(t: Product, children: Array[AnyRef]): Product = {
		...
		try {
			...
			case e: IllegalArgumentException =>
				error("dup: illegal arguments: " + ctor + " " + children.mkString(", ") +
	...
	*/
	...
	child(i: Int, s: Strategy): Strategy =
		new Strategy {
			def
			apply(t: ATerm): Option[ATerm] = {
				if (debug) println("child(" + i + "): " + t + " [" + t.getClass.getSimpleName + "]")
				t match {
					case p: Product =>
						val numchildren = p.productArity
						if ((i < 1) || (i > numchildren)) {
							if (debug) println("child(" + i + ") failure (no ith child): " + p)
							None
						}
						else {
								...
								case ct: ATerm =>
									...
									s(ct) match {
										case Some(ti) =>
											children(i-1) = ti
										case None =>
											if (debug) println("child(" + i + ") failure")
											return None
									}
									val ret = dup(p, children)
									if (debug) println("child(" + i + ") success: " + ret)
									Some(ret)
								case ci =>
									if (debug) println("child(" + i + ") failure (not term): " + ci)
									None
							}
						}

					case c: Constructable =>
						val numchildren = c.nSE

						if ((i < 1) || (i > numchildren)) {
							if (debug) println("child(" + i + ") failure (no ith child): " + c)
							None
						}

						else {
							val elements = c.deconstruct
							val target = elements(i - 1)
							elements(i - 1) match {
								case ct: ATerm =>
									s(ct) match {
										case Some(ti) =>
											if ((ti eq target) || ti == target) {
												if (debug) println("child(" + i + ") nilpotent: " + t)
													Some(t)
											}
											else {
												val children = new Array[AnyRef](numchildren)
												for (j <- 0 until numchildren)
													children(j) =
														if (j != i)
															makechild(elements(j))
														else
															ti

												val ret = c.reconstruct(c, children)
												if (debug) println("child(" + i + ") success: " + ret)
												Some(ret)
											}
					...
					case a =>
						if (debug) println("child(" + i + ") failure (any): " + a)
						None
				}
	...
	*/
	...
	all(s: => Strategy): Strategy =
		new Strategy {
			def
			apply(t: ATerm): Option[ATerm] = {
				if (debug) println("all: " + t + " [" + t.getClass.getSimpleName + "]")
				t match {
					case p: Product =>
						val numchildren = p.productArity
						if (numchildren == 0) {
							if (debug) println("all success (no children): " + p)
							Some(t)
						}
						else {
							val children = new Array[AnyRef](numchildren)
							...
							val ret = dup(p, children)
							if (debug) println("all success: " + ret)
							Some(ret)
						}
					...
					case c: Constructable =>
						...
						if (numchildren == 0) {
							if (debug) println("all success (no children): " + c)
							Some(t)
						}

						else {
							val dElements = c.deconstruct
							val children = new Array[AnyRef](numchildren)
							var nilpotent = true
							for (i <- 0 until numchildren) {
								val e = dElements(i)
							...
							if (nilpotent) {
								if (debug) println("all nilpotent: " + t)
								Some(t)
							}
							else {
								val ret = c.reconstruct(c, children)
					...
					case a =>
						if (debug) println("all success (any): " + a + " [" + a.getClass.getSimpleName + "]")
						Some(a)
				}
			}
		}
	...
	*/
	...
	one(s: => Strategy): Strategy =
		new Strategy {
			def
			apply(t: ATerm): Option[ATerm] = {
				if (debug) println("one: " + t + " [" + t.getClass.getSimpleName + "]")
				t match {
					case p: Product =>
						val numchildren = p.productArity
						for (i <- 0 until numchildren) {
									...
									s(ct) match {
										case Some(nct) => {
											...
											for (j <- i + 1 until numchildren)
												children(j) = makechild(p.productElement(j))
											val ret = dup(p, children)
											if (debug) println("one success: " + ret)
											return Some(ret)
										}
										case None =>
						...
						for (i <- 0 until numchildren) {
							...
							element match {
								case ct: ATerm =>
									s(ct) match {
										case Some(nct) => {
											if ((nct ne element) && nct != element) {
												val children = new Array[AnyRef] (numchildren)
												for (j <- 0 until i)
													children(j) = makechild(elements(j))
												if (ct != nct) {
													children(i) = nct
													for (j <- i + 1 until numchildren)
														children(j) = makechild(elements(j))
												...
												if (debug) println("one nilpotent: " + t)
												return Some(t)
											}
										}
										case None =>
					...
					case _ =>
						if (debug) println("one failure (any)")
						None
				}
			}
		}
	...
	*/
	...
	some(s: => Strategy): Strategy =
		new Strategy {
			def
			apply(t: ATerm): Option[ATerm] = {
				if (debug) println("some: " + t + " [" + t.getClass.getSimpleName + "]")
				t match {
					case p: Product =>
						val numchildren = p.productArity
						if (numchildren == 0) {
							if (debug) println("some failure (no children): " + p)
							None
						}
						else {
							val children = new Array[AnyRef](numchildren)
							...
							if (success) {
								val ret = dup(p, children)
								if (debug) println("some success: " + ret)
								Some(ret)
							}
							else {
								if (debug) println("some failure (no success)")
								None
							}
						}
					...
					case c: Constructable =>
						...
						if (numchildren == 0) {
							if (debug) println("some failure (no children): " + c)
							None
						}
							...
							if (success) {
								if (nilpotent) {
									if (debug) println("some nilpotent: " + t)
									Some(t)
								}
								else {
									val ret = c.reconstruct(c, children)
					...
					case _ =>
						if (debug) println("some failure (any)")
						None
				}
			}
		}
	...
	*/
	def
	rewrite[T <: ATerm](s: => Strategy)(t: T): T = {
		if (debug) println("rewrite: " + t + " [" + t.getClass.getSimpleName + "]")
		s(t) match {
			case Some(t1) =>
				if (debug) println("rewrite success: " + t + " => " + t1)
				t1.asInstanceOf[T]
			case None =>
				if (debug) println("rewrite failure: " + t)
				t
		}
	}
	...
	*/
	def
	collects[T](f: PartialFunction[ATerm,T]): ATerm => Set[T] =
		(t: ATerm) => {
	...
	*/
	def
	collectl[T](f: PartialFunction[ATerm,T]): ATerm => List[T] =
		(t: ATerm) => {
	...
	*/
	def
	count(f: PartialFunction[ATerm,Int]): ATerm => Int =
		(t: ATerm) => {
			var total = 0
			def increment = (v: Int) => total += v

			(everywheretd(query(f andThen increment)))(t)
			total
		}
	...
	*/
	def
	repeat(s: => Strategy, c: => Strategy): Strategy =
		(s <* repeat(s, c)) <+ c
	...
	*/
	def
	repeat1(s: => Strategy, c: => Strategy): Strategy =
		s <* (repeat1(s, c) <+ c)
	...
	*/
	def
	repeatuntil(s: => Strategy, c: => Strategy): Strategy =
		s <* (c <+ repeatuntil(s, c))
	...
	*/
	def
	loop(c: => Strategy, s: => Strategy): Strategy =
	...
	*/
	def
	loopnot(c: => Strategy, s: => Strategy): Strategy =
		c <+ (s <* loopnot(c, s))
	...
	*/
	def
	doloop(s: => Strategy, c: => Strategy): Strategy =
		s <* loop(c, s)
	...
	*/
	def
	loopiter(i: => Strategy, c: => Strategy, s: => Strategy): Strategy =
		i <* loopnot(c, s)
	...
	*/
	def
	loopiter(s: Int => Strategy, low: Int, up: Int): Strategy =
		if (low <= up)
	...
	*/
	...
	/**
		Construct a strategy that tests whether the two sub-terms of a
		pair of terms are equal. Synonym for eql.
	*/
	val
	equal: Strategy = eql
	...
	*/
	...
	issubterm: Strategy =
		strategy {
			case (x: ATerm, y: ATerm) => where(oncetd(term(x)))(y)
		}
	...
	*/
	val
	ispropersubterm: Strategy = not(eql) <* issubterm
	...
	*/
	val
	ispropersuperterm: Strategy = not(eql) <* issuperterm
